...
def gather_and_move_imports(path: Path):
    ...
    head_idx = 0
    while head_idx < len(others) and (
        others[head_idx].strip().startswith("#!")
        or others[head_idx].strip().startswith("#")
        or others[head_idx].strip() == ""
    ):
        head_idx += 1
    ...
    while i < len(lines):
        ...
        if m:
            ...
            if j < len(lines):
                ...
                if not next_line.startswith(indent + " ") and not next_line.startswith(
                    indent + "\t"
                ):
                    pad = indent + "    pass"
                    out.append(pad)
