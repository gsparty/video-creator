...
PYTHON_EXE = (
    None  # leave None to use system 'python' command; set to venv path if desired
)
...
def make_overlay_for(video_path, headline, w=1080, h=1920, fontsize=120):
    ...
    run(
        [
            py,
            str(pathlib.Path(__file__).parent / "overlay_png.py"),
            headline,
            str(overlay_png),
            str(w),
            str(h),
...
def composite(video_path, overlay_png):
    ...
    cmd = [
        ...
        str(overlay_png),
        "-filter_complex",
        "overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/6",
        "-c:v",
        "libx264",
        "-crf",
        "23",
