...
def maybe_upload_to_youtube(video_path, title, desc, privacy="private"):
    ...
    if not upload_script.exists() or not client.exists():
        print(
            "[run_all] YouTube upload skipped (upload_youtube.py or client_secrets.json missing)."
        )
        return False
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="WxH e.g. 540x960 (not enforced by video_builder)",
    )
    p.add_argument(
        "--upload",
        action="store_true",
        help="Attempt YouTube upload if upload script + client_secrets.json are present",
    )
    p.add_argument(
        "--youtube",
        action="store_true",
        help="Also pass flag to call YouTube uploader (requires client_secrets.json)",
    )
    p.add_argument(
        "--privacy",
        type=str,
        default="private",
        help="YouTube privacy: public/private/unlisted",
    )
    args = p.parse_args()
    main(args)
