...
def main():
    ...
    p.add_argument(
        "--background", default="background.mp3", help="background bed (looped)"
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.15,
        help="background volume multiplier when generating bed",
    )
    p.add_argument(
        "--whoosh-vol", type=float, default=0.9, help="whoosh volume multiplier"
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="If >0 override output duration (will trim/extend audio).",
    )
    args = p.parse_args()
    ...
    try:
        ...
        run(
            f'ffmpeg -y -i "{inp}" -vn -ar 44100 -ac 2 -c:a pcm_s16le "{orig_audio}" || echo "no-audio"'
        )
        ...
        if not has_orig:
            ...
            run(
                f'ffmpeg -y -f lavfi -i anullsrc=channel_layout=stereo:sample_rate=44100 -t {duration:.3f} -ar 44100 -ac 2 "{orig_audio}"'
            )
        ...
        if bg.exists():
            ...
            run(
                f'ffmpeg -y -f lavfi -i anullsrc=r=44100:cl=stereo -t {duration:.3f} "{bed_wav}"'
            )
        ...
        if whoosh.exists():
            ...
            run(
                f'ffmpeg -y -f lavfi -i anullsrc=cl=stereo:r=44100 -t {duration:.3f} "{whoosh_placed}"'
            )
        ...
        if ending.exists():
            ...
            run(
                f'ffmpeg -y -f lavfi -i anullsrc=cl=stereo:r=44100 -t {duration:.3f} "{ending_placed}"'
            )
        ...
        run(
            f'ffmpeg -y -i "{inp}" -i "{final_audio}" -map 0:v -map 1:a -c:v libx264 -preset fast -crf 20 -c:a aac -b:a 192k -shortest "{out}"'
        )
        print("Created:", out)
