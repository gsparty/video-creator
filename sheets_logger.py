...
def _open_ws():
    ...
    if not GOOGLE_SERVICE_ACCOUNT_JSON:
        raise RuntimeError("Service account path not set in config.GOOGLE_SERVICE_ACCOUNT_JSON / SERVICE_ACCOUNT_FILE env")
    creds = ServiceAccountCredentials.from_json_keyfile_name(GOOGLE_SERVICE_ACCOUNT_JSON, _scopes)
...
def append_log(row: Dict[str, str]):
    ...
    values = [
        row.get("Timestamp",""), row.get("Region",""), row.get("Status",""),
        row.get("Topic",""), row.get("Title",""), row.get("Hook",""),
        row.get("Body",""), row.get("CTA",""), row.get("Hashtags",""),
        row.get("Caption",""), row.get("Voice",""), row.get("Filepath",""),
        row.get("YouTubeURL","")
    ]
    ws.append_row(values, value_input_option="USER_ENTERED")

def log_post_result(
    region: str, status: str, topic: str, parts: Dict[str,str],
    filepath: str, youtube_url: Optional[str]
):
    ...
    row = {
        ...
        "Body": parts.get("body",""),
        "CTA": parts.get("cta",""),
        "Hashtags": " ".join([t for t in parts.get("caption","").split() if t.startswith("#")]),
